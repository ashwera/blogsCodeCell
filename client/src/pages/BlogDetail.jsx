...
export default function BlogDetail() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        ...
        // Log VIEW event
        const eventsUrl = import.meta.env.PROD
          ? "/api/events"
    ...
    return () => {
      ...
      if (timeSpent > 5) {
        // Only log if they stayed > 5s
        const readUrl = import.meta.env.PROD
          ? "/api/events"
          : "http://localhost:8080/events";
        fetch(readUrl, {
          method: "POST",
  ...
  useEffect(() => {
    if (blog) {
      const setMeta = (name, content) => {
        ...
        if (!element) {
          ...
          element.setAttribute(
            name.includes("og:") ? "property" : "name",
            name,
          );
      ...
      setMeta("og:description", blog.excerpt || blog.content.substring(0, 160));
      setMeta(
        "og:image",
        blog.coverImage ||
          `https://og-image.vercel.app/${encodeURIComponent(blog.title)}.png?theme=dark&md=1&fontSize=100px`,
      );
      setMeta("og:url", window.location.href);
      setMeta("og:type", "article");
      setMeta("article:published_time", new Date(blog.createdAt).toISOString());
      setMeta("article:author", blog.authorName);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <MarkdownPreview content={blog.content} isDark={true} />
        </div>
